...
\begin{align}
\sum_{i = 1}^{m}a_{i}\prod_{j = 1}^{k}s_{ij} \rightarrow & \sum_{i}\sum_{1 \leq s < t \leq k}\frac{\Delta}{2(k-1)}(1 - z_{a_{is}}z_{a_{it}}) + \sum_{i}\sum_{j}\lambda_{ij}s_{ij}x_{ij}
\end{align}
...
& + \left(\frac{a_{1}}{6}\right)^{\nicefrac{1}{3}}\Delta^{\nicefrac{2}{3}}(x_{1}x_{a_{11}} + x_{2}x_{a_{12}} + x_{3}x_{a_{13}}) + \left(\frac{a_{2}}{6}\right)^{\nicefrac{1}{3}}\Delta^{\nicefrac{2}{3}}(y_{4}x_{a_{21}} + x_{2}x_{a_{22}} + z_{5}x_{a_{23}})
\end{align}
}

\refsec
\begin{itemize}
\item Original paper (Eq. 4-5, Example on Eq. 44-45): https://arxiv.org/pdf/1709.02705.pdf
\end{itemize}
