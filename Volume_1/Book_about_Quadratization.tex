...
For $\alpha \ge  0$ we can quadratize a degree-$k$ negative term with one auxiliary variable $b_a$:
...
$\lceil \log \nicefrac{n}{2} \rceil  $ auxiliary qubits per positive monomial.
...
For $\alpha_1 \ge \alpha_2 \ge 0$ we can reduce a degree-$k$ positive term to a degree-$(k-1)$ positive term, plus some extra terms which are at most quadratic:
