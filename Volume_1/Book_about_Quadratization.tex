...
For a symmetric function such that $f\left(|c|\right)=0$ for $c>n$, then with $\max\left( \lceil \log(c)\rceil , \lceil \log(n-c)\rceil  \right)$ auxiliary variables, we have:
...
\subsection{SFR-BCR-10 (Boros, Crama,  Rodr\'{i}guez-Heck, 2018)}
...
{\footnotesize
 \begin{align}
 \left(x_1+x_2\right)\left(z_1+z_2\right)y_3 \rightarrow & \alpha + \alpha^{xx}_{12}x_1x_2 + \alpha^{zz}_{12}z_1z_2 + \alpha^{yy}_{3}y_3 + \alpha^{xx}\left(x_1x_{a_{1}}  + x_2x_{a_{1}}\right) + \alpha^{zx}\left(z_1x_{a_{2}} + z_2x_{a_{2}}\right)\\
 &+ \alpha^{yx}y_3x_{a_{3}} + \alpha^{zz}\left(z_{a_{1}}z_{a_{2}} + z_{a_{1}}z_{a_{3}} + z_{a_{2}}z_{a_{3}}\right)
 \end{align}
 }
