...
\begin{align}
H_{{\rm 6-local}} & =-2b_{1}b_{2}b_{3}b_{4}b_{5}b_{6} + b_5b_6 = 2\sum_{i}^{5}b_{i}-2\sum_{i}^{5}b_{i}b_{6}-2\sum_{i}^6b_{i}b_{a}+10b_{6}b_{a} + b_{5}b_{6}.
\end{align}
has a unique minimum energy of -1 when all $b_i=1$.
...
\examplesec
\begin{align}
b_1b_2b_3 + b_1 + b_2 + b_3 - b_2b_3 \to 1 - b_a + b_a(b_1 + b_2 + b_3) + b_1b_2 + b_1b_3
\end{align}
...
\begin{align}
\pm z_1z_2z_3 &\rightarrow 3 \pm \left( z_1 +  z_2 + z_3 + 2z_a \right) + 2z_a \left(z_1 + z_2 +z_3 \right) +  z_1z_2 + z_1z_3 + z_2z_3.
\end{align}
...
\examplesec
\begin{align}
z_1z_2z_3 - z_1z_2 - z_1 - z_2 \to 3+z_3+2z_a + 2z_a(z_1+z_2+z_3) + z_1z_3 + z_2z_3
\end{align}
...
\item 8 April 2016: published independently by Leib, Zoller, and Lechner in terms of $z$, and without the constant term, see Eq.4 of \cite{Leib2016a,Leib2016}.
\end{itemize}
...
More examples can be found by searching for "flipping" in \href{https://github.com/HPQC-LABS/Book_About_Quadratization/blob/master/everything_else/reproducing.m}{this file}.
...
\noindent We can use any of the PTR methods for even $k$ on the first term, any of the PTR methods for odd $k$ on the second part of the second term, and we can use any of the NTR methods on the first part of the second term. Can be generalized to split into different-degree factors when seeking an "optimum" quadratization. Can be generalized into more splits.
...
\begin{align}
b_1b_2b_3b_4b_5 \rightarrow \prod_{i=1}^{4}b_{i}-\prod_{i=1}^{4}b_{i}(1-b_{5})
\end{align}

% \item Using PTR-Ishikawa, we obtain the following quadratization for the positive term:
% \begin{eqnarray}
% b_{1}b_{2}b_{3}b_{4} \rightarrow  b_{a}(3-2b_{1}-2b_{2}-2b_{3}-2b_{4})+b_{1}b_{2}+b_{1}b_{3}+b_{1}b_{4}+b_{2}b_{3}+b_{2}b_{4}+b_{3}b_{4}
% \end{eqnarray}

% \item Using NTR-ABCG-2, we obtain the following quadratization for the negative term:
% \begin{eqnarray}
% b_{1}b_{2}b_{3}b_{4} \rightarrow 9b_{a}-2(b_{1}+2b_{2}+2b_{3}+2b_{4})b_{a}
% \end{eqnarray}
