...
which has the same global minima as $H_{4-\rm{local}}$ but one less quartic and one less cubic term.
...
\item For $\alpha<0$, we can reduce $\alpha\bar{z}_{1}\bar{z}_{2}...\bar{z}_{k}$ very efficiently so submodular form using \ref{subsec:Negative-Monomial-Reduction}.
