...
\section{Recursive---perturbation---gadget reduction (original)}
...
$\Delta =\Theta\left(\epsilon^{-3}\right)$
...
\subsection{Subdivision Gadget. Oliviera \& Terhal (2005)}
...
% or should the last term be k/2 -1 ?

\noindent Define one qubit $a$ and make the following Hamiltonian is $(\lceil k/2 \rceil +1)$-local:

\begin{align}
\Delta (1-z_a)/2 + |\alpha|(1+z_a)/2 + \sqrt{|\alpha|\Delta /2}\left(\rm{sgn}H_{1,(\nicefrac{k}{2}-\rm{local})} - H_{2,(\nicefrac{k}{2}-\rm{local})}\right)x_a
\end{align}

\noindent The result is a $(\lceil k \rceil /2+1)$-local Hamiltonian with the same low-lying spectrum as $H_{k-\rm{local}}$ for large enough $\Delta$. 


\costsec
$\Delta =\Theta\left(\epsilon^{-2}\right)$.
...
\subsection{Improved Subdivision Gadget. Cao et al. (2013)}
...
\costsec
$\Delta \ge \left( \frac{2|\alpha|}{\epsilon}+1\right)(|\alpha|+\epsilon+22||H_{(k-1)-\rm{local}}  )$
...
\subsubsection{Arbitary real wth only XX and ZZ: Cao et al 2013}
