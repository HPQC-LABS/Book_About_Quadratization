...
All known methods to map the entire spectrum of a Hamiltonian rely on the same basic principle: auxilliary variables are added, to mediate interactions, and constraints are added to restrict the values which the axilliary variables can take. In practice, constraints are implemented as one and two term equations with much stronger energy penalties than those in the equations which are being mapped. As long as the constraints are sufficiently strong, the low energy subspace of the Hamiltonian will be exactly the spectrum of the $k$-local Hamiltonian which has been mapped. Moreover, the location of the original `logical' variables is known and so the resulting configuration of the mapped variables can easily be determined.

%If we want to reproduce the spectrum of  $z_1z_2z_3$, (such that the state $z_1=1,z_2=1,z_3=1$ will have energy $1$ and all others will have energy $0$)  we need to find a quadratic penalty function $P(a,z_1,z_2)$ such that if an auxilliary variable $a$ satisfies $a=z_1z_2$, $P(a,z_1,z_2)=0$, and $P(a,z_1,z_2)>0$ otherwise. If we can find such a function (we explain how to do this is subsection \ref{sub:recursive}), then the desired spectrum will be reproduced in the low energy sector of the following Hamiltonian.

%\begin{equation}
%H_{3-\rm{local}}=z_3\,a+\lambda\,P(a,z_1,z_2)
%\end{equation}
...
To this method is based on taking each higher order term and recursively adding auxilliary variables and a quadratic penalty function to constrain the auxilliary variable reducing the order by one each time until all terms are made quadratic. 

\costsec
\begin{itemize}
\item One auxilliary variable is added each time the order of $b_1b_2...b_n$ is reduced, leading to $n-2$ axilliary terms to reproduce the spectrum .
\end{itemize}

\prossec
\begin{itemize}
\item Works for terms of arbitrary sizes and simple to implement.
\item In principle can be used for more types of equations than just $b_1b_2...b_n$ assuming equations can be found.
\item Relatively sparse connectivity
\end{itemize}

\conssec
\begin{itemize}
\item Not  very symmetric with respect to variables.
\item  Not conducive to specialized hardware design
\end{itemize}

\examplesec

Let us assume we want to reproduce the spectrum of  $H_4=b_1b_2b_3b_4$, we note that $P(a_k,b_i,b_j)=3a_k-2a_k(b_i+b_j)+b_ib_j$ will yield an energy of $E=0$ iff $a_k=b_ib_j$, and $E>0$ otherwise. We therefore observe that, assuming a large value of $\lambda$, the low energy sector of $H_3=b_1b_2b_{a,1} +\lambda(3b_{a,1}-2b_{a,1}(b_3+b_4)+b_3b_4)$ will reproduce the spectrum of $H_4$ with only three local and smaller terms. Performing the same trick again yields a two local Hamiltonian:
...
Similarly to the methods of recursively reducing the order of terms made of products of $b$ operators, gadgets which guarantee that $z_{a,k}=z_iz_j$ can be chained together to make large product terms consisting of $z$. Recall that products of $z$ correspond to parity check or ${\sc xor}$ operations, while products of $b$ correspond to logical ${\sc and}$ operations. The penalty term which guarantee that $z_{a,k}=\pm z_iz_j$ is $P(z_{a,k},z_i,z_j)= \mp z_{a,k}z_iz_j$. Using a gadget for the three local Hamiltonian, the low energy spectrum of Ising terms of any locality can be produced. This method was originally only used to reprduce the ground state of high locality terms, but states of the "wrong" parity ($q_k=\mp z_iz_j$) will all have the same energy as well, so it reproduces the full spectrum. 
...
Let us use this method to reproduce the spectrum of the following five local Hamiltonian $z_1z_2z_3z_4z_5$. We first observe that  the following three local gadget reproduces the spectrum of the three local term $\pm z_1z_2z_3$

\begin{equation}
P_\pm(z_1,z_2,z_3;\lambda)=\lambda\left[z_1z_2+z_2z_3+z_3z_1+2z_a(z_1+z_2+z_3)\right]\mp (z_1+z_2+z_3 +2z_a).
\end{equation}

Using thee copies of this three local gadget as a building block, and using two additional auxilliary variables, the spectrum of the five local term $z_1z_2z_3z_4z_5$ can be reproduced by the following Hamiltonian

\begin{equation}
H_5=P_+(z_1,z_2,z_{a,1};\lambda)+P_+(z_{a,1},z_3,z_{a,2};\lambda)+P_+(z_{a,2},z_4,z_5;\lambda).
\end{equation}
