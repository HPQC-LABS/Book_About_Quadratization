...
\subsection{Constraints and Auxilliary Variables}

\summarysec

All known methods to map the entire spectrum of a Hamiltonian rely on the same basic principle: auxilliary variables are added, to mediate interactions, and constraints are added to restrict the values which the axilliary variables can take. In practice, constraints are implemented as one and two term equations with much stronger energy penalties than those in the equations which are being mapped. As long as the constraints are sufficiently strong, the low energy subspace of the Hamiltonian will be exactly the spectrum of the $k$-local Hamiltonian which has been mapped. Moreover, the location of the original `logical' variables is known and so the resulting configuration of the mapped variables can easily be determined. All methods discussed in this section can be described as variants of this idea, as we discuss later in this section. 

\costsec
\begin{itemize}
\item Variable depending on details of strategy.
\end{itemize}

\prossec
\begin{itemize}
\item All known methods can be described in this way, no other methods are known
\item Has been proven to be universal in the sense that these methods can reproduce not only arbitrary spectra, but to simulate much of classical physics \textcolor{red}{cite Toby Cubitt paper}
\end{itemize}

\conssec
\begin{itemize}
\item Always requires an energy scale significantly larger than in the original Hamiltonian.
\end{itemize}
...
This method is based on taking each higher order term and recursively adding auxilliary variables and a quadratic penalty function to constrain the auxilliary variable, reducing the order by one each time until all terms are made quadratic. 

\costsec
\begin{itemize}
\item One auxilliary variable is added each time the order of $z_1z_2...z_k$ is reduced, leading to $k-2$ axilliary terms to reproduce the spectrum .
\end{itemize}

\prossec
\begin{itemize}
\item Works for terms of arbitrary sizes and simple to implement.
\item In principle can be used for more types of equations than just $z_1z_2...z_k$, assuming the appropriate equations can be found.
...
\item  Not conducive to hardware design
\end{itemize}

\examplesec

Let's assume we want to reproduce the spectrum of  $z_1z_2z_3z_4$, we note that $P(a_k,z_i,z_j)=....$

\textcolor{red}{
subsections  I (NC) need to have:
\begin{itemize}
\item recursive qubo methods (done)
