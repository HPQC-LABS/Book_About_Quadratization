...
For a negative term $-b_{1}b_{2}...b_{k}$, introduce a single auxiliary variable $a$ and make the substitution:
\begin{equation}
-b_{1}b_{2} \ldots b_{k} = \min_a \left\{ (n-1-\sum b_{i})a \right\}.
\end{equation}
...
\begin{equation}
H_{{\rm 2-local}}=2\left( 5a-b_{1}a-b_{2}a-b_{3}a-b_{4}a-b_{5}a-b_{6}a \right) + b_5b_6
\end{equation}
has the same unique minimum energy, and it occurs at the same place (all $b_i=1$), with $a=1$.
...
\begin{equation}
b_{1}b_{2}\ldots b_{k}=\min_{a}\big\{\sum_{i=1}^{k-2}a_{i}(k-i-1+b_{i}-\sum_{j=i+1}^{k}b_{j})\big\}+b_{k-1}b_{k}
\end{equation}
...
\examplesec
\begin{eqnarray}
b_{1}b_{2}b_{3}b_{4} & = & \min_{a}{a_{1}(2+b_{1}-b_{2}-b_{3}-b_{4})+a_{2}(1+b_{2}-b_{3}-b_{4})}+b_{3}b_{4}
\end{eqnarray}
...
\begin{equation}
b_{1}...b_{k} = \min_{a_1, \ldots, a_{n_k}} \left\{ \sum_{i=1}^{n_{k}}a_{i}(c_{i,d}(-\sum_{j=1}^{k}b_{j}+2i)-1)+\sum_{i<j}b_{i}b_{j} \right\}
\end{equation}
...
\examplesec
\begin{eqnarray}
b_{1}b_{2}b_{3}b_{4} & = & \min_{a}(3-2b_{1}-2b_{2}-2b_{3}-2b_{4})a+b_{1}b_{2}+b_{1}b_{3}+b_{1}b_{4}+b_{2}b_{3}+b_{2}b_{4}+b_{3}b_{4}
\end{eqnarray}
...
\begin{equation}
b_{1}b_{2}b_{3}= \min_a \left\{ a-b_{2}a-b_{3}a+b_{1}a+b_{2}b_{3} \right\}
\end{equation}
...
Pick a variable pair $b_{i},b_{j}$ and substitute $b_{i}b_{j}$ for new auxiliary variable $a_{i,j}$.
Enforce equality in the ground states by adding some scalar multiple of $b_{i}b_{j}-2b_{i}a_{i,j}-2b_{i}a_{i,j}+3a_{i,j}$ or similar.
...
\begin{equation}
b_{1}b_{2}b_{3}+b_{1}b_{2}b_{4}\mapsto b_{3}a+b_{4}a+b_{1}b_{2}-2b_{1}a-2b_{1}a+3a
\end{equation}
...
\item $\alpha_{H}>0$ 
\begin{equation}
\sum_{H\in\mathcal{H}}\alpha_{H}\prod_{j\in H}b_{j}=\min_{a}(\sum_{H\in\mathcal{H}}\alpha_{H})a\prod_{j\in C}b_{j}+\sum_{H\in\mathcal{H}}\alpha_{H}(1-a)\prod_{j\in H\setminus C}b_{j}
\end{equation}
\item $\alpha_{H}<0$
\begin{equation}
\sum_{H\in\mathcal{H}}\alpha_{H}\prod_{j\in H}b_{j}=\min_{a}\sum_{H\in\mathcal{H}}\alpha_{H}(1-\prod_{j\in C}b_{j}-\prod_{j\in H\setminus C}b_{j})a
\end{equation}
\end{enumerate}
...
\begin{eqnarray}
b_{1}b_{2}b_{3}+b_{1}b_{2}b_{4} & \mapsto & 2a_{1}b_{1}+(1-a_{1})b_{2}b_{3}+(1-a_{1})b_{2}b_{4}\\
 & = & 2a_{1}b_{1}+b_{2}b_{3}+b_{2}b_{4}-a_{1}b_{2}b_{3}-a_{1}b_{2}b_{4}
\end{eqnarray}
now we can use \ref{subsec:Negative-Monomial-Reduction}:

\begin{eqnarray}
-a_{1}b_{2}b_{3}-a_{1}b_{2}b_{4} & \mapsto & 2a_{2}-a_{1}a_{2}-a_{2}b_{2}-a_{2}b_{3}+2a_{2}-a_{1}a_{2}-a_{2}b_{2}-a_{2}b_{4}\\
 & = & 4a_{2}-2a_{1}a_{2}-2a_{2}b_{2}-a_{2}b_{3}-a_{2}b_{4}.
\end{eqnarray}
...
\item  At most $kt$ for a $k$-local Hamiltonian of $t$ terms.
\end{itemize}

\prossec
\begin{itemize}
\item Works for arbitrary $k$ {\color{red} Nick, are there methods that DON'T work for arbitrary $k$?} and simple to implement.
\item In principle can be used for more types of equations than just $b_1b_2...b_n$  assuming equations can be found.{\color{red} Nick, why is this a "pro", what is the advantage over other methods?}
\item Gives a 2-local Hamiltonian with sparse connectivity relative to {(\color{red} some?)} other methods.
\end{itemize}

\conssec
\begin{itemize}
\item Not  very symmetric with respect to variables.
\item  Not conducive to specialized hardware design {\color{red} (what specialized hardware design?) } 
\end{itemize}

\examplesec

Let us assume we want to reproduce the spectrum of  $H_4=b_1b_2b_3b_4$, we note that $P(a_k,b_i,b_j)=3a_k-2a_k(b_i+b_j)+b_ib_j$ will yield an energy of $E=0$ iff $a_k=b_ib_j$, and $E>0$ otherwise. We therefore observe that, assuming a large enough value of $\lambda$, the low energy sector of $H_3=b_1b_2b_{a,1} +\lambda(3b_{a,1}-2b_{a,1}(b_3+b_4)+b_3b_4)$ will reproduce the spectrum of $H_4$ with terms that are $k$-local with $k\le 3$. Performing the same trick again yields a $2$-local Hamiltonian:

\begin{equation}
H_2=b_1a_2+ \lambda\left(3b_{a,2}-2b_{a,2}(b_2+b_{a,1})+b_2b_{a,1}\right) +\lambda\left(3b_{a,1}-2b_{a,1}(b_3+b_4)+b_3b_4\right)
\end{equation}


\refsec
\begin{itemize}
\item \textcolor{red}{add later, references to many D-Wave papers including Biamonte and Bian papers, as well as Perdomo-Ortiz paper}
\end{itemize}
...
The penalty term which guarantees that $b_{a,k}=\pm b_ib_j$ is $P(b_{a,k},b_i,z_j)= \mp b_{a,k}b_ib_j$. Using a gadget for the three local Hamiltonian, the low energy spectrum of Ising terms of any locality can be produced. This method was originally only used to reprduce the ground state of high locality terms, but states of the "wrong" parity ($q_k=\mp b_ib_j$) will all have the same energy as well, so it reproduces the full spectrum. 
...
\prossec
\begin{itemize}
\item Natural transmon implementation.
\item Relatively low degree of connectivity.
\end{itemize}
...
Let us use this method to reproduce the spectrum of the following five local Hamiltonian $b_1b_2b_3b_4b_5$. We first observe that  the following three local gadget reproduces the spectrum of the three local term $\pm b_1b_2b_3$
...
\begin{equation}
H_5=P_+(b_1,b_2,b_{a,1};\lambda)+P_+(b_{a,1},b_3,b_{a,2};\lambda)+P_+(b_{a,2},b_4,b_5;\lambda).
\end{equation}

\refsec
\begin{itemize}
\item \textcolor{red}{add later, cite second Lechner paper, plus NC work on three local gadget}
\end{itemize}
...
Parity adiabatic quantum computing (PAQC) is an AQC archetecture in which logical qubits are mapped onto a highly non-local logical space of a Hamiltonian. These methods allow for high order terms to be directly implemented, but also require high locality terms to be implemented. While these archetectures are not strictly a mapping from devices with high locality to two local ones, they may be built upon any gadget which has the same ground state manifold as $\prod_iz_i$. In other words, even PAQC schemes can reprduce the entire spectrum of a Hamiltonian, even if the gadgets which they are built on top of do not.

\costsec
\begin{itemize}
\item In the original (Lechner-Hauke-Zoller) scheme \textcolor{red}{citation}, an $n$ body PAQC constraint which is capable of reproducing the full spectrum requires a $2n$ local gadget which reproduces only the ground state manifold of $\prod_i^{2n}z_i$. 
\item Since $n$ logical operators must intersect for an $n$ local gadget, at least a gadget which reproduces the ground state manifold of the ground state manifold of $\prod_i^{n}z_i$ is required.
\end{itemize}

\prossec
\begin{itemize}
\item Connection to stablizers allows for novel decoding\textcolor{red}{citations}.
\item Natural for producing highly connected Hamiltonians.
...
\item Potentially difficult to embed arbitrary sparse Hamiltonians efficiently.
\end{itemize}
...
\begin{equation}
H_4=m\,z_{1,2,3,4}+\lambda\,(H_{LHZ}-z_{1,2}z_{3,4}z_{1,2,3,4}),
\end{equation}
...
\begin{equation}
H_3=m\,z_{1,2,3}+\lambda\,(H_{LHZ}-z_{1}z_{2,3}z_{1,2,3}).
\end{equation}

\refsec
\begin{itemize}
\item \textcolor{red}{add later, cite LHZ and Simon Benjamin stabilizer paper}
\end{itemize}
...
\begin{itemize}
\item recursive qubo methods
\item chains of 3 local gadgets (thing in second Lechner paper)
\item our methods based on symmetry
\item Simon Benjamin Stabilizer idea and multidimensional LHZ (one section)
\item Possibly others: need to check how distinct some of the work Alejandro Perdomo-Ortiz and Jacob Biamonte have done is, whether these should be their own subsections or just references in exiting ones
\end{itemize}
}
...
\begin{align}
H_{k-\rm{local}} = \sum_i\prod_j s_{ij} + H_{(k-1)-\rm{local}}. \label{eq:klocalInKempeMethod}
\end{align}

\noindent Define auxiliary qubits labeled by $a_{ij}$ and make the transformation:

\begin{align}
\sum_i\prod_j s_{ij} \mapsto \frac{\Delta}{4}\left( 9 - \sum_i \left( \sum_j  z_{a_{ij}} \right)^2 - \sum_j z_{a_{ij}}^2 + \frac{1}{\sqrt[3]{6\Delta}} x_{a_{ij}}  \right)+ \Lambda.
\end{align}

\noindent The result will be a $(k-1)$-local Hamiltonian with the same low-lying spectrum as $H_{k-\rm{local}}$ for large enough $\Delta$. 

\costsec

The number of auxiliary qubits needed is the number of $s_{ij}$ matrices in Eq. \eqref{eq:klocalInKempeMethod}.
