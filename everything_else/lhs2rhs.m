...
% While a similar funciton lhsrhs.m can only be called from a parent function (FindReqdDelta)
%
% e.g.
%         [LHS, RHS] = lhs2rhs(-1,'zzz',1e10,'P(3->2)DC2')
%         refers to quadratize -z1*z2*z3 using P(3->2)DC2 with Delta = 1e10


    n = length(operators);      % number of operators
    x = [0 1 ; 1 0]; y = [0 -1i ; 1i 0]; z = [1 0 ; 0 -1];  % Pauli matrices
    ...
    if strcmp(name_of_quadratization, 'P(3->2)-DC2') || strcmp(name_of_quadratization, 'P(3->2)DC2')
        ...
        % create matrices for each operator in order and store them in S array
        for ind = 1:n
            if operators(ind) == 'x'
        ...
        % operators on auxiliary qubits are defined separately since they are different for various gadgets
