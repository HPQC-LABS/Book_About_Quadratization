...
parfor Delta = 1e9:1e9:1e13                         % this parfor loop is able to run on Octave, but not on MATLAB
...
for s2 = 1:3
for s3 = 1:3
    parfor i = -308:308     % this parfor loop is able to run on Octave, but not on MATLAB
    Delta = 10^i;
    combinations(n_combination) = {['-' S{s1} S{s2} S{s3}]};     % terms that being tested
    [LHS,RHS] = lhs2rhs(['-' S{s1} S{s2} S{s3}],Delta,'P(3->2)OT');
    if isnan(RHS) == 0
      [V_RHS,E_RHS] = eig(RHS);
      [V_LHS,E_LHS] = eig(LHS);
      [E_RHS,index] = sort(diag(E_RHS)); V_RHS = V_RHS(:,index);
...
delta_required(delta_required == 0) = nan;
combinations
delta_required     % get delta_required and corresponding combinations as outputs
