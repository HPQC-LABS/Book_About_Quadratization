...
isequal(sorted_L(:,1:repeat_times:end),round(abs(V_RHS(:,unique(ind_evals_R)))))                     % gives 1
...
% Failed example: it has 7 eigenvectors matching ( V_RHS(1-7) ), but still returns false due to small errors in V_RHS. We need to find a way to ignore those errors.

[LHS,RHS] = lhs2rhs('zxz',4.5e12,'P(3->2)DC2');
tol = 1e-03;
[V_RHS,E_RHS] = eig(RHS);
[V_LHS,E_LHS] = eig(LHS);
E_RHS = diag(E_RHS);
...
sorted_L = L(:,ind_evecs_L);
repeat_times = sum(ind_evals_R == ind_evals_R(1,1))*sum(ind_evecs_R == ind_evals_R(1,1));    % times of eigenvectors repeat in sorted_L
isequal(sorted_L(:,1:repeat_times:end),round(abs(V_RHS(:,unique(ind_evals_R)))))
