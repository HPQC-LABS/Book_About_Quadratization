...
%% DC2:
%% has tested zzz, zxx (4e12), zxz (4.5e12) etc., but need loop to find appropriate and more precise Delta value for zzz, zzx etx.

[LHS,RHS] = lhs2rhs('zzz',2e13,'P(3->2)DC2'); restriction = 1e-03;
[V_RHS,E_RHS] = eig(RHS); [V_LHS,E_LHS] = eig(LHS); E_RHS = diag(E_RHS)'; E_LHS = diag(E_LHS)';
...
[ind_LHS, ind_RHS] = find(E_diff < restriction); % that tells us which columns of E_LHS and E_RHS match, respectively
...
dist = 2*ones(max(ind_LHS),max(ind_RHS)); % set dist to 2 defaultly so that we can find matching eigenvectors easily in next step
...
% dist_ind_LHS and dist_ind_RHS refers to rows and columns of dist, which have elements smaller than 1e-3 (i.e., dist_ind_LHS and dist_ind_RHS tells us which columns of V_LHS match the columns of V_RHS)
[dist_ind_LHS, dist_ind_RHS] = find(dist < restriction);
