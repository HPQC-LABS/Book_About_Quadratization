...
\author{\vspace{2mm}Nike Dattani}
\email{nik.dattani@gmail.com}
%\affiliation{Oxford University, Department of Mathematics}
%\affiliation{McMaster University, Department of Chemistry and Chemical Biology}
\affiliation{Harvard Smithsonian Center for Astrophysics, Cambridge, Massachusetts}
...
\vspace{-5mm}  %%%%%%%%%%%%%%%%%%%%%%%% UNCOMMENT WHEN ADDING BACK AUTHORSHIP
...
%\vspace{5mm} \indent  
\noindent\rule{\textwidth}{0.4pt}
\vspace{-1.25mm} %%%%%%%%%%%%%%%%%%%%%%%% REMOVE WHEN ADDING BACK AUTHORSHIP
...
\noindent where each $b_i$ can either be 0 or 1, both never go below the value of -2, and all minima have the form $(b_1,b_2,b_3,b_4) = (1,1,1,0)$.  Therefore if we are interested in the ground state of a discrete function of degree $k$, we may optimize either function and get exactly the same result. { \textbf{Part \ref{partDiagonal}}} gives more than 40 different ways to do this, almost all of them published within the last 5 years. 

%\vspace{15mm}

%\vspace{2mm} %%%%%%%%%%%%%%%%%%%%%%%% REMOVE WHEN ADDING BACK AUTHORSHIP
\vspace{0.5mm}
\noindent\rule{\textwidth}{0.4pt}
%\vspace{0.5mm}
%\vspace{2mm} %%%%%%%%%%%%%%%%%%%%%%%% REMOVE WHEN ADDING BACK AUTHORSHIP
...
\vspace{-2mm}
\\
\indent The binary variables $b_i$ can be either of the eigenvalues of the matrix $b$ below, which is related to the Pauli $z$ matrix by $z=2b-\openone$. Other Pauli matrices are listed below:
...
\subsection{NTR-AC (Asymmetric cubic reduction)}
...
\refsec
\begin{itemize}
\item Original paper (Theorem 5): \cite{Boros2018QuadratizationsOS}, Also in (Theorem 9): \cite{Boros2018boundisPaper}.
\end{itemize}

\newpage

% Elisabeth's first email to Nike has a PDF which says PTR-BCR4 and PTR-BCR-5 are the exact same, so I've commented out the entire PTR-BCR-5 below !!!! Now what used to be PTR-BCR-6 is PTR-BCR-5.

% HAVE TO COMMENT IT OUT, BUT HAVEN'T DONE IT YET BECAUSE IT WOULD MESS UP THE PAGE NUMBERS IN WHAT I'M EMAILING ELISABETH.
...
\subsection{PTR-A (Asymmetric reduction)}
...
\altformsec
\begin{align}
f\left(b_{1},b_{2},\ldots,b_{n}\right)&\rightarrow \binom{-(c+1)+\sum_{i}b_{i}-\sum_{i}^{m-1}2^{i-1}b_{a_{i}}+\left(1+2^{m-1}\right)b_{a_{m}}}{2}
\end{align}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
% ELISABETH SAYS THAT THIS IS THE SAME AS SFR-BCR-5. THEOREM 6 IN [26] IS THE SAME AS THEOREM 9 IN [17], AND THERE IS SOME ERROR IN THE [17] VERSION WHICH WAS CORRECTED IN [26] SO THATS WHY THEY LOOKED DIFFERENT.
%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{itemize}
\item A positive monomial with $n$ variables cannot be quadratized with fewer than $\lceil \log(n)\rangle - 1$ auxiliary variables, unless there is some extra deduction we can make about the optimization problem, as in for example deduc-reduc (Corollary 1 from \cite{Boros2018boundsPaper}).
\item ALCN (at least $c$ out of $n$) and ECN (exact $c$ out of $n$) functions also cannot be quadratized with fewer than $\lceil \log(n)\rangle - 1$ auxiliary variables (Corollaries 2 and 3 from \cite{Boros2018boundsPaper}).
...
\subsection{ABCG Reduction (Anthony-Boros-Crama-Gruber, 2017)}
...
\item Richard wrote the original versions of the following sections: (1) Deduc-Reduc, (2) ELC Reduction, (3) Groebner Bases, (4) Split Reduction, (5) NTR-KZFD, (6) NTR-AC, (7) PTR, (8) PTR-Ishikawa, (9) PTR-KZ, (10) PTR-A, (11) Bit flipping, (12) RBS, and (13) FGBZ. 
\item Richard also wrote the "Further Example" of Deduc-Reduc in the Appendix.
% need to check some of these, for example, was NTR-KZFD there in the original version?, which Boros 2018 ones did he actually add? I know NTR-AC I told him to add from Wikipedia and he didn't add much so I had to do things.
\end{itemize}
...
\item We thank Paul Ayers of McMaster University who begged us over and over again for early copies of this Book when we kept saying it was still not presentable, and when we finally gave it to him, he asked us copious numbers of questions which we always answered and then he said that he would use some of our ideas for a paper about Hamiltonian transformations in quantum chemistry applications. He also used references given to him by us, for his 2017 NSERC grant application. 
