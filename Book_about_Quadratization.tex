...
\subsection{Application of ELM (new in this book, 2018)}
...
% = b1   + b2  -b1b3 -b2b3 - b2ba  +b3ba
% = b1   + b2  -b1b3 -b2b3 - b2ba  -b3ba + b3ba

% This means the first sum only goes up to k-1, 
% the second term goes up to k-1,
% the third term does not have b1ba, so it starts from i=2
...
\begin{itemize}
\item By starting with \eqref{eqn:NTR-GBP,3}, flipping $b_a$ (i.e. setting $b_a \rightarrow 1- \bar{b}_a$ and relabelling $b_a \rightarrow \bar{b}_a$ since $b_a$ does not appear anywhere else in the function being quadratized), and then re-labeling $b_1 \rightarrow b_2$, we see that NTR-GBP can actually be derived from NTR-ABCG with $k=3$).
\end{itemize}
