...
Determine the $k$-local term, $H_{k-\rm{local}}$, whose degree we wish to reduce, and factor it into two commuting factors: $H_{k^\prime-\rm{local}}H_{(k-k^\prime)-\rm{local}}$. Separate all terms that are at momst $(k-1)$-local into ones that commmute with the $k$-local term  and ones that anti-commute with the $k$-local term:

\begin{align}
%H_{k-\rm{local}} + H_{(k-1)\rm{-local}}^{\rm{commuting}} + H_{(k-1)\rm{-local}}^{\rm{anti-commuting}} 
 H_{ <k \rm{-local}}^{\rm{commuting}} +H_{ <k \rm{-local}}^{\rm{anti-commuting}}  + \alpha H_{k-\rm{local}}
\end{align}
	
Introduce one auxiliary qubit labeled by $a$ and the Hamiltonian:

\begin{align}
\alpha x_a +  H_{ <k \rm{-local}}^{\rm{commuting}} + z_aH_{ <k \rm{-local}}^{\rm{anti-commuting}} 
\end{align}
...
\costsec
\begin{itemize}
\item 1 auxiliary qubit to reduce $k$-local term to 1-local
\item Raises the $k$-locality of $H_{ <k \rm{-local}}^{\rm{anti-commuting}}$
\end{itemize}
...
\item If the portion of the Hamiltonian that does not commute with the $k$-local term has termms of degree $k-1$, they will all become $k$-local, so there is no guarantee that this method reduces $k$-locality.
...
\begin{align}
4z_5 -3 x_1 + 2z_1y_2x_5 + 9x_1x_2x_3x_4 -x_1y_2z_3x_5 \rightarrow \alpha %Use real numbers instead of alphas !
\end{align}
