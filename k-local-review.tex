...
\title{Quadratization in Pseudo-Boolean Optimization and Ground-State Quantum Computing}
...
Implementing $\lambda H_{3-\rm{SAT}}$, creates a situation where $z_3$ is a `flag' for $z_1=-1$ and $ z_2=-1$ in other words $z_3$ is constrained to be $-1$ in the low energy manifold iff $z_1=-1$ and $ z_2=-1$ and $+1$ otherwise. It follows from the universality of $3-\rm{SAT}$ that these `flag' clauses can be combined to map any spin Hamiltonian. To do this, we also need anti-ferromagnetic couplings to express the `negated' variable, to do this, we define,
...
\end{align}
 ...
 As an explicit example, consider reproducing the spectrum of $z_1z_2z_3$. In this case we need to assign a higher energy to the $(+1,+1,+1)$, $(-1,-1,+1)$, $(-1,+1,-1)$, and  $(+1,-1,-1)$ states. A flag ($z_{f}$) which is forced into a higher energy state iff these conditions are satisfied can be constructed from two instances of $H_{3-\rm{SAT}}$ and an auxilliary qubit, combining these leads to

\begin{align}
H_{3-\rm{SAT}}(z_1,z_2,z_3)=\lambda[\sum_{i=1}^3 H_{\rm{AF}}(z_i,z_{\neg i})+\sum_{i=1}^8 H_{\rm{AF}}(z_{a_i},z_{\neg a_i})+ H_{\rm{AF}}(z_{f},z_{\neg f})\nonumber \\
+H_{3-\rm{SAT}}(z_{\neg 1},z_{\neg 2},z_{\neg a_1})+H_{3-\rm{SAT}}(z_{a_1},z_{\neg 3},z_{f})+H_{3-\rm{SAT}}(z_{1},z_{2},z_{\neg a_2})+H_{3-\rm{SAT}}(z_{a_2},z_{3},z_{\neg f})\nonumber \\
+H_{3-\rm{SAT}}(z_{1},z_{2},z_{\neg a_3})+H_{3-\rm{SAT}}(z_{a_3},z_{\neg 3},z_{f})+H_{3-\rm{SAT}}(z_{\neg 1},z_{\neg 2},z_{\neg a_4})+H_{3-\rm{SAT}}(z_{a_4},z_{3},z_{\neg f})\nonumber \\
+H_{3-\rm{SAT}}(z_{1},z_{\neg 2},z_{\neg a_5})+H_{3-\rm{SAT}}(z_{a_5},z_{3},z_{f})+H_{3-\rm{SAT}}(z_{\neg 1},z_{2},z_{\neg a_6})+H_{3-\rm{SAT}}(z_{a_6},z_{\neg 3},z_{\neg f})\nonumber \\
+H_{3-\rm{SAT}}(z_{1},z_{2},z_{\neg a_7})+H_{3-\rm{SAT}}(z_{a_7},z_{\neg 3},z_{f})+H_{3-\rm{SAT}}(z_{\neg 1},z_{ \neg 2},z_{\neg a_8})+H_{3-\rm{SAT}}(z_{a_8},z_{3},z_{\neg f})]-z_{f}.
\end{align}
...
\begin{equation}
H_\textrm{5-local} = z_1z_2z_3z_4z_5 -2 z_1z_4z_5 +3z_1z_3 -z_5 + 9
\end{equation}

The full spectrum of $\pm z_1z_2z_3$ is reproduced by:
...
\begin{equation}
H_{2-\rm{local}}=P_+(z_1,z_2,z_{a_1};\lambda)+P_+(z_{a_1},z_3,z_{a_2};\lambda)+P_+(z_{a_2},z_4,z_5;\lambda).
\end{equation}
...
%
\begin{equation}
H_{2-\rm{local}}=z_{a_1}-z_{a_2}+z_{a_3}-z_{a_4}+ \lambda H_{4-\rm{count}},
\end{equation}
%
...
%
\begin{equation}
H_{2-\rm{local}}=-\frac{1}{2}z_{a_4}+ \lambda H_{4-\rm{count}}.
\end{equation}
...
\subsection{$(3\rightarrow2)$-DC (Duan, Chen, 2011)} %The paper says s_i are Pauli matrices, but I'd bet that they just have to be commuting, well they don't have the s_i^2 terms, which would not be = 1 if they were general commuting matrices.
...
\noindent The result will be a 2-local Hamiltonian whose low-lying spectrum is equivalent to the spectrum of $H_{k-\rm{local}}$ to within $\epsilon$ as long as $\Delta=\Theta\left(\epsilon^{-3}\right)$. 
...
\summarysec
 ...
 For any 3-local term (product of Pauli matrices $s_i$) in the Hamiltonian, we can define three auxiliary qubits labeled by $a_{i}$  and  make the transformation:                                                                                                                                      
%Group all $3$-local terms together and express their sum as a sum of products of commuting matrices $s_{ij}$:
...
\noindent The result will be a 2-local Hamiltonian whose low-lying spectrum is equivalent to the spectrum of $H_{3-\textrm{local}}$ to within $\epsilon$ as long as $\Delta=\Theta\left(\epsilon^{-3}\right)$. 
...
The authors describe the method as a ``transfer'' of non-locality from the original Hamiltonian, into a unitary transforation operator. 
...
\conssec
\begin{itemize}
\item Can introduce new 3-local terms. 
\item
\end{itemize}
