...
\subsection{NTR-RBL-(3$\rightarrow$2) (Rochetto, Benjamin, Li, 2016)}
...
\subsection{NTR-RBL-(4$\rightarrow$2) (Rochetto, Benjamin, Li, 2016)}
...
\subsection{PTR-RBL-(3$\rightarrow$2) (Rochetto, Benjamin, Li, 2016)}
...
\subsection{PTR-RBL-(4$\rightarrow$2) (Rochetto, Benjamin, Li, 2016)}
...
For a sum of terms that are $k$-local, each term $j$ written as a product $H_{1j}H_{2j}$, introduce $N_{\rm core}$ ``core'' auxiliary qubits and $N_{\rm direct}$ ``direct'' auxiliary qubits.
...
\begin{array}{ccl}
\displaystyle
\sum_{j}a_j H_{1j} H_{2j} & \rightarrow & \alpha \sum_{ij} \left(1-z_{a_{ij}} z_{a_i} + \beta_j  \left(H_{1j}  - H_{2j}\right) x_{w_i^{(j)}} \right) + \alpha_2\left(\sum_i(1-z_{a_i}) + \sum_{ii^\prime}\left(1-z_{a_i} z_{a_i^\prime}\right)\right)  \\[0.1in]
\end{array}
\end{equation}
