...
\item Further discussion: \cite{Anthony2016}.
\end{itemize}
...
b_{1}b_{2}\cdots b_{k}	\rightarrow\alpha^{b}\sum_{i}b_{i}+\alpha^{b_{a_{1}}}\sum_{i}b_{a_{i}}+\alpha^{b_{a_{2}}}b_{a_{m}}+\alpha^{bb}\sum_{ij}b_{i}b_{j}+\alpha^{bb_{a_{1}}}\sum_{i}\sum_{j}^{m-1}b_{i}b_{a_{j}}+\\
	\alpha^{bb_{a_{2}}}\sum_{i}b_{i}b_{a_{m}}+\alpha^{b_{a_{1}}b_{a_{1}}}\sum_{ij}^{m-1}b_{a_{i}}b_{a_{j}}+\alpha^{b_{a_{1}}b_{a_{2}}}\sum_{ij}^{m-1}b_{a_{i}}b_{a_{m}},
\end{gathered}
\end{align}
...
\altnamesec
\begin{itemize}
\item "Reduction by Minimum Selection" \cite{Gallagher2011}.
\end{itemize}
...
\section{Methods that quadratize MULTIPLE terms with the SAME auxiliaries (Case 2: Parity Functions)}
...
\section{Methods that quadratize MULTIPLE terms with the SAME auxiliaries (Case 3: Arbitrary Functions)}
...
\item 8-body to 2-body transformation can be accmplished in 1 step, rather than a 1B1 gadget which would take 6 steps or an SD + $(3\rightarrow2)$ gadget combination which would take 4 steps.
\end{itemize}
...
\begin{align}
\prod_i^3 \sum_j \alpha_{ij}s_{i} \rightarrow \alpha^I + \alpha_i^{ss} \sum_i \left(\sum_j \alpha_{ij}s_{ij}\right)^2 + \alpha_i^{sx}\sum_i \sum_j \alpha_{ij} s_{ij}x_{a_i} + \alpha^{zz} \sum_{ij} z_{a_i}z_{a_j} 
\end{align}


\begin{align}
\alpha^I    &= \frac{1}{8\Delta} \\
...
\begin{align}
\alpha\prod_i^3 s_{i} \rightarrow \alpha^I + \alpha^s s_3 + \alpha^z z_a +\alpha^{ss} \left(s_1 + s_2 \right)^2 + \alpha^{sz}s_3z_a  +  + \alpha^{sx}\left(s_1x_a + s_2x_a  \right) 
\end{align}

{\scriptsize
\begin{align}
\alpha^I    &= -\frac{1}{2\Delta} \\
...
{\small
\begin{align}
x_1z_2y_3 - 3x_1x_2y_4 + z_1x_2 &\rightarrow \alpha^I +  \alpha^z(z_{a_1} + z_{a_2}) + \alpha^y(y_3 + y_4) +\alpha^{zx}_{12}z_1x_2+   \alpha^{zx}z_2x_{a_1} +\alpha^{xx}_{11} x_1x_2\\
...
\begin{align}
\prod_i^3 s_{i} \rightarrow \alpha^I + \alpha_i^{ss} \sum_i s_i^2 + \alpha_i^{sx}\sum_i  s_ix_{a_i} + \alpha^{zz} \sum_{ij} z_{a_i}z_{a_j} 
\end{align}
...
{\footnotesize
\begin{align}
\alpha\prod_i^3 s_{i} \rightarrow \alpha^I + \alpha_1^{s} s_1^2 + \alpha_2^s s_2^2 + \alpha_3^s s_3 + \alpha_a^z z_a + \alpha_{12}^{ss} s_1s_2 + \alpha_{13}^{ss} s_1^2s_3 + \alpha_{23}^{ss} s_2^2s_3 + \alpha_{3a}^{sz} s_3z_a + \alpha_{1a}^{sx}s_1x_a+\alpha_{2a}^{sx}s_2x_a   
\end{align}
}
