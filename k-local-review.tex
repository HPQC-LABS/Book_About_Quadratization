...
\begin{align}
s_1s_2+s_2s_3+s_3s_4 -4s_1s_2s_3 & \hspace{5mm} \textrm{(cubic)},\label{eq:intro_quantum_cubic}
\end{align}  
...
%A quantum computer whose Hamiltonian is at least quadratic in $x$ and $z$ can find this ground state with only polynomial runtime overhead over the best alternative algorithm to find that solution. 
 The optimization problems of Eqs. \eqref{eq:intro_cubic}-\eqref{eq:intro_quadratic} are specific cases of Eq. \eqref{eq:intro_quantum_cubic} where only $z$ matrices are present. 
...
\subsection{Reduction by Minimum Selection}

\summarysec

This method can also be used to rewrite positive cubic terms in terms of 6 quadratic terms.

The identity is given by:
\begin{align}
b_{1}b_{2}b_{3} &= \min_{b_a} - \left( b_a + b_1 + b_2 + b_3 \right) + b_a \left( b_1 + b_2 + b_3 \right) + b_1 b_2 + b_2 b_3 + b_3 b_1
\end{align}
...
\conssec
\begin{itemize}
\item Introduces all 6 possible non-submodular terms.
\end{itemize}

\refsec
\begin{itemize}
\item Original introduction by Kolmogorov and Zabih: \cite{Kolmogorov2004}.
\end{itemize}
...
\subsection{$(3\rightarrow2)$-DC  (Duan, Chen, 2011)} %The paper says "the terms in the product have to commute, but it's not clear if they mean each of the 3 terms of the product, or just the terms inside those terms, which would be much better.
...
\subsection{$(3\rightarrow2)$-DC2 (Duan, Chen, 2011)} %The paper says s_i are Pauli matrices, but I'd bet that they just have to be commuting, well they don't have the s_i^2 terms, which would not be = 1 if they were general commuting matrices.
...
\subsection{$(3\rightarrow2)$-KKR (Kempe, Kitaev, Regev, 2004)}
...
\subsection{$(3\rightarrow2)$-OT (Oliveira-Terhal, 2005)}
...
\subsection{1B1-OT (Oliveira \& Terhal, 2008)}
...
\subsection{SD-OT (Oliveira \& Terhal, 2008)}
...
\subsection{DR-JF (Jordan \& Farhi, 2008)}
...
\subsection{DR-BFBD (Brell, Flammia, Bartlett, Doherty, 2011)}
...
\noindent is transformed into the following 2-body Hamiltonian:
...
\item Formalization of pseudo-Boolean gadgets in Green's quantum language. \cite{Biamonte2008a}.
