...
\usepackage[colorlinks,urlcolor=blue,bookmarksopen=true]{hyperref}
\usepackage{bookmark}
...
\vspace{-10mm}
\title{Quadratization in Discrete Optimization and Quantum Mechanics}

\author{Richard Tanburn}
%\email{richard.tanburn@gmail.com}
\affiliation{Oxford University, Department of Mathematics}

\author{Nicholas Chancellor}
%\email{nicholas.chancellor@gmail.com}
\affiliation{Durham University, Department of Physics}

%\author{Jacob D.~Biamonte$^{a,b}$}
%\affiliation{$^{a}$Deep Quantum Labs, Skolkovo Institute of Science and Technology, 3 Nobel Street, 
%Moscow, Russia 121205}
%\affiliation{$^{b}$Institute for Quantum Computing, University of Waterloo, 200 University Ave West, Waterloo, Canada ON N2L 3G1}
...
\affiliation{Oxford University, Department of Mathematics}
\affiliation{McMaster University, Department of Chemistry and Chemical Biology}
...
%\vspace{5mm} \indent  
\noindent\rule{\textwidth}{0.4pt}
\indent When optimizing discrete functions, it is often easier when the function is qudaratic than if it is of higher degree. But notice that the cubic and quadratic funcitons:
...
\noindent where each $b_i$ can either be 0 or 1, both never go below the value of -1, and all minima have the form $(b_1,b_2,b_3,b_4) = (1,1,1,b_4)$.  Therefore if we are interested in the ground state of a discrete function of degree $k$, we may optimize either function and get exactly the same result. { \textbf{Part \ref{partDiagonal}}} gives more than 15 different ways to do this. 
...
The binary variables $b_i$ can be either of the eigenvalues of the matrix $b$ below, which is related to the Pauli $z$ matrix by $z=2b-\openone$. Other Pauli matrices are listed below:
...
\noindent Any Hermitian $2\times 2$ matrix can be written as a linear combination of the Pauli matrices $z,x,y,\openone$, so we can therefore describe the Hamiltonian of any number of spin-$\nicefrac{1}{2}$ particles by a function of Pauli matrices acting on each particle, for instance:

\vspace{-5mm}

\begin{align}
z_1x_2+x_2y_3+z_3z_4 -4y_1y_2z_3 & \hspace{5mm} \textrm{(cubic)},\label{eq:intro_quantum_cubic}
\end{align}  
...
%A quantum computer whose Hamiltonian is at least quadratic in $x$ and $z$ can find this ground state with only polynomial runtime overhead over the best alternative algorithm to find that solution. 
 The optimization problems of Eqs. \eqref{eq:intro_cubic}-\eqref{eq:intro_quadratic} are specific cases of the type in Eq. \eqref{eq:intro_quantum_cubic} where only $z$ matrices are present. 
...
\item Only works for Hamiltonians whose minimization corresponds to solving systems of discrete equations, as the method only preserves roots, not minima.
\end{itemize}
...
\subsection{Application of ELM (Present Work)}
...
%\refsec
%\begin{itemize}
%\item The method is new in the present work.
%\end{itemize}
...
For a negative term $-b_{1}b_{2}...b_{k}$, introduce a single auxiliary variable $b_a$ and make the substitution:
\begin{equation}
-b_{1}b_{2} \ldots b_{k} \rightarrow-\sum_{i}b_{i}-\sum_{i}b_{i}b_{k}-\sum_{i}b_{i}b_{a}+(k-1)b_{k}b_{a}.
\end{equation}
...
-b_{1}b_{2}\ldots b_{k}&\rightarrow(k-1)b_{k}b_{a}-\sum_{i}b_{i}(b_{a}+b_{k}-1)\\
                       & = (k-2)b_a - \sum_i^{k-1}b_i\left( b_a + b_k -1 \right)
\end{align}
...
%\altformsec  SEE the Anthony2016 paper, they write it with a 2 on the outsize and a 1/2 on the inside.
%\begin{align}
%\end{align}
...
\subsection{Asymmetric Cubic Reduction}
...
\refsec
\begin{itemize}
\item Discussion: \cite{Ishikawa2011}, \cite{Kahl2011}.
\end{itemize}
...
\refsec
\begin{itemize}
\item Original paper (Theorem 5): \cite{Boros2018QuadratizationsOS}.
\end{itemize}
...
\subsection{Reduction by Minimum Selection (in terms of $z$)}

\summarysec

The formula is almost the same as in the version PTR-KZ in terms of $b$, but with a factor of 2, a change of sign for the linear terms, and a slight change in the constant term:
...
\refsec
\begin{itemize}
\item 31 March 2016 published by Chancellor, Zohren, and Warburton without the constant term: \cite{Chancellor2016b}.
\item 8 April 2016 published independently by Leib, Zoller, and Lechner without the constant term \cite{Leib2016a,Leib2016}.
\end{itemize}
...
\subsection{SFR-BCR-1 (Boros, Crama, Rodr\'{i}guez-Hector, 2018)}
...
\altformsec
\begin{align}
f\left(b_{1},b_{2},\ldots,b_{n}\right) &\rightarrow \left(-(c+1)+\sum_{i}b_{i}-\left(2c-2^{m-1}+1\right)b_{a_{m}}-\sum_{i}^{m-1}2^{i-1}b_{a_{i}}\right)^{2}
\end{align}
...
\subsection{SFR-BCR-2 (Boros, Crama, Rodr\'{i}guez-Hector, 2018)}
...
{\scriptsize
\begin{align}
f(b)\rightarrow \left(\left(m-1\right)-\sum_{i}b_{i}-\left(2(n-m)-2^{\lceil\log(n-m)\rceil}+1\right)b_{a_{\lceil\log(n-m)\rceil+1}}-\sum_{i}^{\lceil\log(n-m)\rceil}2^{i-1}b_{a_{i}}\right)^{2}
\end{align}
}
...
\subsection{SFR-BCR-3 (Boros, Crama, Rodr\'{i}guez-Hector, 2018)}

\summarysec
% actually they say =1, not "non-zero"

Any $n$-variable symmetric function $f\left(b_1,b_2,\ldots b_n\right)\equiv f(b)$  that is non-zero only when $\sum b_i = c$ where $\nicefrac{n}{2}\le c \le n$, can be quadratized with $m=\lceil \textrm{log}_2c \rceil +1$ auxiliary variables $f\rightarrow$:

{\scriptsize
\begin{align}
\frac{1}{2}\left(\sum_{i}b_{i}-\left(m-2^{c}\right)b_{a_{c+1}}-(m+1)(\left(1-b_{a_{c+1}}\right)-\sum_{i}^{c}2^{i-1}b_{a_{i}}\right)\left(\sum_{i}b_{i}-\left(m-2^{c}\right)b_{a_{c+1}}-(m+1)(\left(1-b_{a_{c+1}}\right)-\sum_{i}^{c}2^{i-1}b_{a_{i}}-1\right)
\end{align}
}
...
\altformsec
\begin{align}
f\left(b_{1},b_{2},\ldots,b_{n}\right)&\rightarrow \binom{\frac{1}{2}\left(\sum_{i}b_{i}-\left(m-2^{c}\right)b_{a_{c+1}}-(m+1)(\left(1-b_{a_{c+1}}\right)-\sum_{i}^{c}2^{i-1}b_{a_{i}}\right)}{2}
\end{align}
...
\subsection{SFR-BCR-4 (Boros, Crama, Rodr\'{i}guez-Hector, 2018)}

\summarysec

Any $n$-variable symmetric function $f\left(b_1,b_2,\ldots b_n\right)\equiv f(b)$  that is non-zero only when $\sum b_i = c$ where $0\le c \le\nicefrac{n}{2} $, can be quadratized with $m=\lceil \textrm{log}_2(n-c) \rceil +1$ auxiliary variables $f\rightarrow$:

{\scriptsize
\begin{align}
\frac{1}{2}\left(\left(c-1\right)-\sum_{i}b_{i}-\left(2(n-c)-2^{m-1}+1\right)b_{a_{m}}-\sum_{i}^{m-1}2^{i-1}b_{a_{i}}\right)\left(\left(c-1\right)-\sum_{i}b_{i}-\left(2(n-c)-2^{m-1}+1\right)b_{a_{m}}-\sum_{i}^{m-1}2^{i-1}b_{a_{i}}-1\right)
\end{align}
}
...
\subsection{SFR-BCR-5 (Boros, Crama, Rodr\'{i}guez-Hector, 2018)}
...
\refsec
\begin{itemize}
\item 2018, original paper (Theorem 9): \cite{Boros2018QuadratizationsOS}.
\end{itemize}

\newpage

\subsection{SFR-BCR-6 (Boros, Crama, Rodr\'{i}guez-Hector, 2018)}
...
\begin{itemize}
\item There exist symmetric functions on $n$ variables for which no quadratization can be done without at least $\Omega\left(\sqrt{n}\right)$  auxiliary variables (Theorem 5.3 from \cite{Anthony2014,Anthony2016}.  %$2\log_2\left( 1+\sqrt{257}  \right)-8$.
...
\subsection{FGBZ Reduction (Fix-Gruber-Boros-Zabih, 2011)}
...
Determine the $k$-local term, $H_{k-\rm{local}}$, whose degree we wish to reduce, and factor it into two commuting factors: $H_{k^\prime-\rm{local}}H_{(k-k^\prime)-\rm{local}}$, where $k^\prime$ can be as low as 0. Separate all terms that are at momst $(k-1)$-local into ones that commmute with one of these factors (it does not matter which one, but without loss of generality we assume it to be the $(k-k^\prime)$-local one)  and ones that anti-commute with it:
...
\begin{align}
\vspace{-1mm}
4z_5 -3 x_1 + 2z_1y_2x_5 + 9x_1x_2x_3x_4 -x_1y_2z_3x_5 \rightarrow 9x_{a_{1}} + 4z_{a_4}z_5   -3z_{a_3}x_1  -z_{a_3}x_{a_4} a3  +2x_{a_3}x_5  
\end{align}
...
\subsection{P$(3\rightarrow2)$-DC  (Duan, Chen, 2011)} %The paper says "the terms in the product have to commute, but it's not clear if they mean each of the 3 terms of the product, or just the terms inside those terms, which would be much better.
...
\begin{itemize}
\item 1 auxiliary qubit for each group of 3-local terms that can be factored into three 1-local factors. %\eqref{eq:klocalInKempeMethod}.
...
%\alpha^s    &= \alpha\left(\frac{1}{4\Delta^{\nicefrac{2}{3}}} - 1 \right) \\
%\alpha^z    &= \alpha\left(\frac{1}{4\Delta^{\nicefrac{2}{3}}} - 1 \right) \\
%\alpha^{ss} &= \frac{1}{\Delta^{\nicefrac{1}{3}}} \\
%\alpha^{sz} &= \frac{\alpha}{4\Delta^{\nicefrac{2}{3}}} \\ 
%\alpha^{sx} &=  \frac{1}{\Delta^{\nicefrac{2}{3}}} \\
\end{align}
%}
...
For a sum of $M$ terms that are $k$-local, each subdivided into parts $A_{a_j}$ and $B_{b_j}$ with $j=1,2,\cdots,M$, introduce two sets of ancilla qubits: one set being the ``core ancilla qubits", which consists of $C$ qubits that are all-to-all connected via the Hamiltonian $H_\mathcal{C}$, the second set being the ``direct ancilla qubits", which are qubits that couple the $A_{a_j}$ and $B_{b_j}$ of each $k$-local term with the core ancilla qubits. For each term $j$ we introduce $R$ direct ancilla qubits, labelling them as $w_i^{(j)}$ with $i=1,2,\cdots,R$. Specifically, we accomplish the following transformation:

\begin{equation}
\begin{array}{ccl}
\displaystyle
H_\text{targ}=H_\text{else}+\sum_{j=1}^M\gamma_j A_{a_j}\otimes B_{b_j} & \rightarrow & \displaystyle \tilde{H}=
\underbrace{\frac{J}{2} \sum_{j=1}^{M}\sum_{i=1}^{R} \sum_{c\in \mathcal{C}}(I-z_{w_i^{(j)}} z_c)+ H_\mathcal{C}}_{H} \\[0.1in]
& & \displaystyle +\underbrace{H_\text{else}+\sum_{j=1}^M \beta_j \sum_{i=1}^{R} \left(A_{a_j}  - B_{b_j}\right)\otimes x_{w_i^{(j)}}}_{V}
\end{array}
\end{equation}
where $H_\mathcal{C}$ is defined as the Hamiltonian of a set of $C$ all-to-all coupled qubits 
\begin{equation}\label{eq:Hcdef}
	H_\mathcal{C}=\frac{J}{2}\sum_{c\in \mathcal{C}}(I-z_c) + \frac{J}{2}\sum_{c,c' \in \mathcal{C}}(I-z_c z_{c'}).
\end{equation}

If we would like the spectrum of $H_\text{targ}$ to be close to that of $\tilde{H}$, say within difference $O(\epsilon)$. Then for any $d\in(0,1)$ we can choose 
\begin{align}
\label{eq:RC}
	R & \in \Omega\left( \max\left\{
			\epsilon^{-\frac{2}{d}}, 
			\left(\frac{\|H_\text{else}\|^2}{2M^4 \gamma_{\text{max}}}\right)^{\frac{1}{d}},
			\left(M^3 \epsilon^{-2} \right)^{\frac{1}{1-d}}
		\right\}\right),\\ 
        \label{eq:CC}
        C & \in \Omega \left( M^3 R^d \,\epsilon^{-1}\right), \\
        \beta_j, J & \in O(\epsilon) \label{eq:eps}
\end{align}
where $\gamma_\text{max}=\max_{1,\cdots,M}|\gamma_j|$. 
...
\item The construction only describes the asymptotic scaling of the parameters rather than concrete assignments of them. More work is needed for finding tight non-asymptotic error bounds in perturbative expansion.
\end{itemize}
...
% Lechner mentions sigma_x a lot in his talk, and also in his answer to Mohseni's question
% LHZ can do k-local to 4-local (?) with maybe (N choose k) auxiliaries. (N choose 2) for 2-loca, (N choose 3) for 3-local, etc. look into 
% Lechner's talk says that Rochetto's scheme can add k-locals to LHZ lattice by sacrificing not being able to do all-2-all
...
\item We thank Paul Ayers of McMaster University, who has been a major supporter of this review paper since the idea was presented to him in October 2016. At many points during the preparation of this review, we had prioritized other commitments and put preparation of this review aside. Paul's frequent encouragement was often what got us working on this review again. Without him, we are not certain this paper would have been completed by this time (or ever!). Paul also provided to us other interesting quantum mechianics problems in quantum chemistry and particle physics which served as a major motivation for continuing with this review when writing it was in a state of hiatous. These problems gave us more ideas about what trade-offs in quadratization might be important for some of its potential users in quantum chemistry and particle physics.
