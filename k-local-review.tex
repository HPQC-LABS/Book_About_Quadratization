...
For a sum of $M$ terms that are $k$-local, each subdivided into parts $A_{a_j}$ and $B_{b_j}$ with $j=1,2,\cdots,M$, introduce two sets of ancilla qubits: 
...
For each term $j$ we introduce $R$ direct ancilla qubits, labelling them as $w_i^{(j)}$ with $i=1,2,\cdots,R$. 
Specifically, we accomplish the following transformation:

\begin{equation}
\begin{array}{ccl}
\displaystyle
H_\text{targ}=H_\text{else}+\sum_{j=1}^M\gamma_j A_{a_j} B_{b_j} & \rightarrow & \displaystyle \tilde{H}=
\underbrace{\frac{J}{2} \sum_{j=1}^{M}\sum_{i=1}^{R} \sum_{c\in \mathcal{C}}(I-z_{w_i^{(j)}} z_c)+ H_\mathcal{C}}_{H} \\[0.1in]
& & \displaystyle +\underbrace{H_\text{else}+\sum_{j=1}^M \beta_j \sum_{i=1}^{R} \left(A_{a_j}  - B_{b_j}\right) x_{w_i^{(j)}}}_{V}
\end{array}
\end{equation}
...
If we would like the spectrum of $H_\text{targ}$ to be close to that of $\tilde{H}$, say within difference $O(\epsilon)$. 
Then for any $d\in(0,1)$ we can choose 
\begin{align}
\label{eq:RC}
