...
\documentclass[english,notitlepage,showpacs,preprintnumbers,amsmath,amssymb,aps,prl,nofootinbib,12pt,superscriptaddress]{revtex4-1}
...
\item To solve every possible $m$-variable deduction for $n$ total variables would involve $\binom{n}{m} 2^m$ evaluations of the objective function, whereas $2^n$ evaluations is enough to solve the entire problem. We therefore choose $m \lll n$.
...
\subsection{Gruber Positive Term Reduction}

\summarysec

Suppose we have a monomial $b_1b_2b_3 \ldots b_k$ with $k > 2$.
Let $X = \sum^k_{i=1} b_i$.

Let $m = \lceil \frac{k}{4} \rceil, N = n - 2m$ and
$Y = \sum^m_{j=2} b'_j$ where $b'_1, \ldots, b'_m$ are auxiliary variables.
Note that the $j$ index starts at 2.
Then:
...
Then:
\begin{eqnarray}
b_1 b_2 b_3 b_4 = \min_{b'} \frac{1}{2}
\left( b_1 + b_2 + b_3 + b_4 - 2b'_1 \right)
\left( b_1 + b_2 + b_3 + b_4 - 2b'_1 - 1 \right)
\end{eqnarray}
...
\subsection{Logarithmic Positive Monomial Reduction}

\summarysec

Suppose we have a monomial $b_1b_2b_3 \ldots b_k$.
Pick $n$ minimal such that $k < 2^{n+1}$ and let $N = 2^{n+1} - k$.

For convenience, let $X = \sum^k_{i=1} b_i$.
...
Then:
\begin{eqnarray}
b_1 b_2 b_3 b_4 = \min_{b'_1, b'_2} \frac{1}{2}
\left( 4 + b_1 + b_2 + b_3 + b_4 - b'_1 - 2b'_2 \right)
\left( 3 + b_1 + b_2 + b_3 + b_4 - b'_1 - 2b'_2 \right)
\end{eqnarray}
...
\subsection{Reduction by Minimum Selection (Kolmogorov \& Zabih, 2004)}
...
The formula is almost the same as in the version in terms of $b$, but with a factor of 2, a change of sign for the linear terms, and a slight change in the constant term:

\begin{align}
\pm z_1z_2z_3 &= 3 \pm \left( z_1 +  z_2 + z_3 + z_a \right) + 2z_a \left(z_1 + z_2 +z_3 \right) +  z_1z_2 + z_1z_3 + z_2z_3.
\end{align}
...
\subsection{Another Cubic Reduction}

\summarysec
\begin{align}
-b_{1}b_{2}b_{3}&= \min_{b_a} b_a \left( -b_1 + b_2 + b_3  \right) -b_1b_2 - b_1b_3 + b_1 
\end{align}
